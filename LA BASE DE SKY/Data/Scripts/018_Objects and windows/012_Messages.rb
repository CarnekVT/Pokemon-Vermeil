...
def pbMessageRunControls(text, controls, state, msgwindow)
  controls.length.times do |i|
    control = controls[i][0]
    param = controls[i][1]
    case control
...
def pbMessageDisplayFrame(msgwindow, controls, letterbyletter, commandProc, state)
  ...
  if state[:shout] && state[:shout] != 0
    state[:shout] = (state[:shout] * -0.9).floor
    if state[:atTop]
      msgwindow.y = state[:shout]
    else
      msgwindow.y = Graphics.height - (msgwindow.height + state[:shout])
    end
  end
...
def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil)
  ...
  text = pbMessageRunControls(text, controls, state, msgwindow)
  if state[:startSE]
