...
class PokemonPartyPanel < Sprite
  ...
  def draw_hp
    ...
    if @pokemon.able?
      ...
      color_index, next_color_index, blend_alpha = pbHPBarZoneInfo(@pokemon.hp, @pokemon.totalhp, 3)
      @overlaysprite.bitmap.blt(UI_HP_BAR_X, UI_HP_BAR_Y, @hpbar.bitmap, Rect.new(0, color_index * 8, w, 8))
      if blend_alpha > 0 && color_index != next_color_index
        @overlaysprite.bitmap.blt(UI_HP_BAR_X, UI_HP_BAR_Y, @hpbar.bitmap, Rect.new(0, next_color_index * 8, w, 8), blend_alpha)
      end
    end
  end
