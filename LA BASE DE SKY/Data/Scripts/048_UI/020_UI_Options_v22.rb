...
class UI::Options < UI::BaseScreen
  ...
  def initialize(in_load_screen = false, menu = :options_menu)
    @in_load_screen = in_load_screen
    @menu = menu
    $PokemonSystem.vsync = $PokemonSystem.vsync_initial_value? if $PokemonSystem
...
if Settings::USE_NEW_OPTIONS_UI
  ...
  MenuHandlers.add(:options_menu, :vsync, {
    ...
    "set_proc"    => proc { |value, scene|
      ...
      next if $PokemonSystem.update_vsync(value)
      $PokemonSystem.vsync = old_value
      next if !scene || !scene.sprites[:options_list]
      scene.sprites[:options_list].get_values
      scene.refresh
    }
  })
