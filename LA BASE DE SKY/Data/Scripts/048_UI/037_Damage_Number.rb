...
class DamageNumberSprite < Sprite
  ...
  def create_bitmap
    ...
    temp.font.size += 4 if @category == :critical
    ...
    self.bitmap.font.size += 4 if @category == :critical
  ...
  def update
    ...
    fade_start = [[DamageNumberSettings::FADE_START, 0.0].max, 0.99].min
...
class Battle::Scene
  ...
  def pbHPChanged(battler, oldHP, showAnim = false)
    if DamageNumberSettings::ACTIVE && $PokemonSystem.damage_numbers == 0 && battler
      ...
      if diff != 0
        ...
        dni_create_sprite(battler, diff.abs, category) if category != :heal || DamageNumberSettings::SHOW_HEAL
      end
    end
    ...
    if DamageNumberSettings::ACTIVE && $PokemonSystem.damage_numbers == 0
      targets.each do |data|
        ...
        if battler
          ...
          if diff > 0
            amount = diff
            amount = battler.damageState.calcDamage if battler.damageState && battler.damageState.calcDamage > amount

            # Determinar si es crítico
            category = :physical
            category = :critical if battler.damageState && battler.damageState.critical
  ...
  def dni_create_sprite(battler, amount, category)
    ...
    Console.echoln "[DNI] Cat: #{category} | Amt: #{amount}" if DamageNumberSettings::DEBUG_LOGS
