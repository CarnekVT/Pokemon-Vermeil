...
MenuHandlers.add(:options_menu, :animated_sprites, {
  ...
  "order"       => 35,
  "type"        => (defined?(Settings::USE_NEW_OPTIONS_UI) && Settings::USE_NEW_OPTIONS_UI) || !defined?(EnumOption) ? :array : EnumOption,
...
class PokemonSummary_Scene
  UI_SPRITE_CONSTRICT_W = 208
  UI_SPRITE_CONSTRICT_H = 164
...
class PokemonPokedexInfo_Scene
  ...
  def pbUpdateDummyPokemon
    ...
    @sprites["infosprite"].pbSetDisplay([INFO_SPRITE_X, INFO_SPRITE_Y, 208, 200], species_id)
    @sprites["formfront"].pbSetDisplay([FORM_FRONT_X, FORM_FRONT_Y, 200, 196], species_id) if @sprites["formfront"]
    if @sprites["formback"] && @sprites["formfront"]
      ...
      @sprites["formback"].pbSetDisplay([FORM_BACK_X, FORM_BACK_Y, 200, 196], species_id, true)
    end
  end
end
...
class PokemonStorageScene
  ...
  def pbUpdateOverlay(selection, party = nil)
    ...
    @sprites["pokemon"].pbSetDisplay([POKEMON_SPRITE_X, POKEMON_SPRITE_Y, 168]) if pokemon
