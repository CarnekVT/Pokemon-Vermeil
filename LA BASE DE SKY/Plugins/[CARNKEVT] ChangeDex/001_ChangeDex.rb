...
module CarnekChangeData
  ...
  RECOVERY_PATH = "Data/ChangeDex/pokemon_changes.recovery.json"
  SCHEMA_VERSION = 5
  @applied = false
  @applying = false
  ...
  def self.active_json_path
    main_ok = File.file?(JSON_PATH)
    recovery_ok = File.file?(RECOVERY_PATH)
    return JSON_PATH if main_ok && !recovery_ok
    return RECOVERY_PATH if recovery_ok && !main_ok
    return nil if !main_ok && !recovery_ok
    begin
      return RECOVERY_PATH if File.mtime(RECOVERY_PATH).to_f > File.mtime(JSON_PATH).to_f
    rescue StandardError
    end
    return JSON_PATH
  end
  ...
  def self.json_signature
    path = active_json_path
    return nil if !path || !File.file?(path)
  ...
  # Evolution tuples are special: every record is species, method, parameter,
  # but methods with no parameter need an explicit empty third slot. The generic
  # csv() helper intentionally drops empty fields, which used to shift a valid
  # value such as GOLEM,Trade,,GOLEM,Item,THUNDERSTONE into
  # GOLEM/Trade/GOLEM + ITEM/THUNDERSTONE and caused false "New Evo" entries.
  def self.evolution_method_uses_parameter?(method)
    id = method.to_s.strip
    return false if id.empty? || id.casecmp("none") == 0
    begin
      evo = GameData::Evolution.try_get(id.to_sym) if defined?(GameData::Evolution)
      return !evo.parameter.nil? if evo && evo.respond_to?(:parameter)
    rescue StandardError
    end
    # Safe fallback for the stock no-parameter methods if the Evolution registry
    # is not available yet. Unknown/custom methods default to having a parameter.
    no_param = %w[
      Trade Happiness HappinessMale HappinessFemale HappinessDay HappinessNight
      HappinessMove HappinessMoveType Beauty Shedinja
    ]
    return !no_param.any? { |m| m.casecmp(id) == 0 }
  end

  def self.parse_evolution_rows(raw)
    tokens = if raw.is_a?(Array)
               raw.flatten.map { |v| v.nil? ? "" : v.to_s.strip }
             else
               raw.to_s.split(",", -1).map { |v| v.to_s.strip }
             end
    rows = []
    i = 0
    while i < tokens.length
      species = tokens[i].to_s.strip
      i += 1
      break if species.empty? && i >= tokens.length
      next if species.empty?
      method = (i < tokens.length) ? tokens[i].to_s.strip : "None"
      method = "None" if method.empty?
      i += 1
      param = nil
      if evolution_method_uses_parameter?(method)
        param = (i < tokens.length) ? tokens[i].to_s.strip : ""
        i += 1
      elsif i < tokens.length && tokens[i].to_s.empty?
        # Consume the explicit PBS placeholder for a parameterless method.
        i += 1
      end
      rows << [species.to_sym, method.to_sym, int_or_symbol(param)]
    end
    return rows
  end
  ...
  def self.assign(entry, field, raw)
    ...
    when "Evolutions"
      entry.instance_variable_set(:@evolutions, parse_evolution_rows(raw))
    when "EVs"
      parts = csv(raw); h = {}
  ...
  def self.apply!
    ...
    return false if !runtime_data_ready?
  ...
  # v0.11.19: ChangeDex is a runtime overlay. It must never mutate GameData
  # while Essentials is compiling OR serializing PBS. In some Essentials/load
  # orders plugins are evaluated before Compiler exists, so relying only on a
  # prepend installed at plugin load time is not sufficient.
  def self.compiler_suspended?
    return @compiler_suspend_depth.to_i > 0
  end

  # Fallback guard that works even when the Compiler module was not yet defined
  # when this plugin loaded. This is only evaluated while the overlay is stale,
  # so normal hot GameData getters do not pay a permanent stack-walk cost.
  def self.compiler_call_context?
    return true if compiler_suspended?
    begin
      if Kernel.respond_to?(:caller_locations)
        Kernel.caller_locations(2, 64).each do |loc|
          path = (loc.path rescue "").to_s
          label = (loc.base_label rescue loc.label rescue "").to_s
          next if path =~ /ChangeDex/i
          return true if path =~ /Compiler/i
          return true if label =~ /\A(?:compile|validate_(?:all_)?compiled|write_(?:PBS|pbs|pokemon|regional_dexes|all)|pbWriteCsvRecord|get_all_PBS_file_paths)\b/i
        end
      else
        caller(2, 64).each do |line|
          txt = line.to_s
          next if txt =~ /ChangeDex/i
          return true if txt =~ /Compiler/i
        end
      end
    rescue StandardError
    end
  ...
  # v0.11.22: Essentials clears a whole GameData registry at the beginning of
  # compile_PBS_file_generic. Compiling only pokemon_Vermeil.txt (or another
  # single layered PBS file) therefore leaves Species containing only that
  # layer and makes valid cross-layer references such as GOLBAT -> CROBAT fail.
  # Always expand partial compiles of the layered core registries to the whole
  # sibling PBS group before Essentials clears/validates the registry.
  def self.layered_pbs_base_for(game_data)
    return "pokemon"   if defined?(GameData::Species) && game_data == GameData::Species
    return "moves"     if defined?(GameData::Move)    && game_data == GameData::Move
    return "abilities" if defined?(GameData::Ability) && game_data == GameData::Ability
    return "items"     if defined?(GameData::Item)    && game_data == GameData::Item
    return nil
  rescue StandardError
    return nil
  end

  def self.expand_layered_pbs_paths(game_data, paths)
    base = layered_pbs_base_for(game_data)
    original = (paths || []).flatten.compact.map { |p| p.to_s }.reject { |p| p.empty? }
    return original if !base

    dirs = original.map { |p| File.dirname(p) }.uniq
    dirs = ["PBS"] if dirs.empty?
    candidates = []
    dirs.each do |dir|
      Dir.glob(File.join(dir, "#{base}*.txt")).each do |path|
        next if !File.file?(path)
        filename = File.basename(path)
        if base == "pokemon"
          # These are separate GameData registries / compilers and must never be
          # fed to GameData::Species.compile_pokemon.
          next if filename =~ /\Apokemon_(?:forms|metrics)(?:_|\.txt)/i
        end
        candidates << path
      end
    end
    return original if candidates.empty?

    candidates.uniq!
    canonical = "#{base}.txt".downcase
    candidates.sort_by! do |path|
      name = File.basename(path).downcase
      [name == canonical ? 0 : 1, name]
    end

    # Preserve any explicit path outside the discovered sibling set.
    original.each do |path|
      begin
        expanded = File.expand_path(path)
        next if candidates.any? { |candidate| File.expand_path(candidate) == expanded }
      rescue StandardError
  ...
  def self.runtime_data_ready?
    return false if compiler_suspended? || compiler_call_context? || bulk_loading?
    # These files are mandatory products of a complete Essentials v21.1 PBS
    # compilation. Their absence is a strong signal that registries are still
    # being rebuilt and must not receive the JSON overlay yet.
    ["Data/species.dat", "Data/moves.dat", "Data/abilities.dat", "Data/regional_dexes.dat"].each do |path|
  ...
  def self.begin_compiler_guard!
    if @compiler_suspend_depth.to_i <= 0
      @compiler_suspend_depth = 0
      @compiler_guard_failed = false
    end
    @compiler_suspend_depth += 1
    return @compiler_suspend_depth
  end

  def self.end_compiler_guard!(success = true)
    @compiler_guard_failed = true if !success
    @compiler_suspend_depth = [@compiler_suspend_depth.to_i - 1, 0].max
    return @compiler_suspend_depth if @compiler_suspend_depth > 0
    failed = (@compiler_guard_failed == true)
    @compiler_guard_failed = false
    mark_stale!
    # Never reapply merely because an inner compiler method returned. If an
    # outer compiler/write routine is still on the stack, defer until the next
    # real runtime GameData access.
    apply! if success && !failed && runtime_data_ready?
    return 0
  end
  ...
  def self.ensure_applied
    # v0.11.24: ChangeDex is a runtime-only overlay. It never installs or
    # touches Compiler hooks. During any compiler call it is a complete no-op.
    return if @applying || @applied
    return if compiler_call_context? || bulk_loading?
    return if !runtime_data_ready?
    apply!
  end
  ...
  def self.install_hooks!
    # v0.11.24: intentionally empty. Previous builds aliased
    # GameData::Species/Move/Ability get/try_get/each/load. Even when those
    # aliases only forwarded to the original methods, they still inserted
    # ChangeDex into Essentials/Sky compiler validation and could bypass later
    # plugin method chains. Runtime activation is now done from Game.initialize.
    return true
  end
...
module VermeilChangeDex
      ...
      File.open(file_path, "r:utf-8") do |f|
        f.each_line do |raw_line|
          ...
          if clean[/^BaseStats\s*=\s*(.*)$/i]
            ...
            data[:evolutions] = CarnekChangeData.parse_evolution_rows($1.strip)
          end
        end
      end
    end
        ...
        f.each_line do |line|
          ...
          if clean[/^\[([^\]]+)\](?:!exclude)?$/]
            ...
            @canon_move_data[current_id] ||= {
              ...
              :total_pp => nil, :target => nil, :priority => nil, :function => nil, :effect_chance => nil, :flags => []
            }
            next
          end
          ...
          elsif clean[/^Target\s*=\s*(.*)$/i]; data[:target] = $1.to_s.strip.to_sym
  ...
  MOVE_FUNCTIONAL_FIELDS = ["Type","Category","Power","Accuracy","TotalPP","Target","Priority","FunctionCode","EffectChance","Flags"] unless const_defined?(:MOVE_FUNCTIONAL_FIELDS)
  ...
  def self.move_snapshot(move_data)
    ...
    target = begin
      raw = move_data.respond_to?(:target) ? move_data.target : nil
      raw.nil? ? nil : raw.to_sym
    rescue StandardError
      nil
    end
    return {
      ...
      :target => target, :priority => (move_data.priority.to_i rescue 0), :function => (move_data.function_code.to_s rescue ""),
      :effect_chance => (move_data.effect_chance.to_i rescue 0),
      :flags => ((move_data.flags || []).map { |x| x.to_s }.sort rescue [])
    }
  end

  def self.move_real_pbs_diff?(move_id, move_data = nil)
    ...
    [:type, :dmg_class, :power, :accuracy, :total_pp, :target, :priority, :function, :effect_chance].each do |k|
  ...
  def self.move_behavior_changed?(move_id)
    change = entity_change("moves", move_id)
    return false if !change.is_a?(Hash)
    explicit = change["_behaviorChanged"]
    return true if explicit == true || explicit.to_s.downcase == "true"
    # Imported PBS comparisons often contain _beforeText/_afterText/_notes as
    # explanatory metadata. If the row also contains PBS fields, the live
    # canon-vs-current comparison above is authoritative; stale metadata must
    # not create a fake Move Change entry. Rows made only of behavior text are
    # still valid script-side reworks.
    has_pbs_fields = change.keys.any? { |k| MOVE_FUNCTIONAL_FIELDS.include?(k.to_s) }
    return false if has_pbs_fields
    before = change["_beforeText"].to_s.strip
    after  = change["_afterText"].to_s.strip
    notes  = change["_notes"].to_s.strip
    if !before.empty? && !after.empty?
      norm = proc { |t| t.to_s.gsub(/\s+/, " ").strip.downcase }
      return true if norm.call(before) != norm.call(after)
    end
  ...
  def self.move_data_changed?(move_id, move_data = nil)
    return true if move_real_pbs_diff?(move_id, move_data)
    return true if move_behavior_changed?(move_id)
    return false
  rescue StandardError
    return false
  end
  ...
  def self.move_imported_metadata_stale?(move_id)
    data = entity_change("moves", move_id)
    return false if !data.is_a?(Hash)
    has_pbs_fields = data.keys.any? { |k| MOVE_FUNCTIONAL_FIELDS.include?(k.to_s) }
    return false if !has_pbs_fields
    return !move_real_pbs_diff?(move_id)
  rescue StandardError
    return false
  end
  ...
  def self.reworked_move_before_text(move_id)
    data = entity_change("moves", move_id)
    raw = data["_beforeText"].to_s.strip if data.is_a?(Hash) && !move_imported_metadata_stale?(move_id)
  ...
  def self.reworked_move_after_text(move_id)
    data = entity_change("moves", move_id)
    raw = (data.is_a?(Hash) && !move_imported_metadata_stale?(move_id)) ? data["_afterText"].to_s.strip : ""
    return localized_behavior_text("moves", move_id, "AfterText", raw)
  end

  def self.reworked_move_notes(move_id)
    data = entity_change("moves", move_id)
    raw = (data.is_a?(Hash) && !move_imported_metadata_stale?(move_id)) ? data["_notes"].to_s.strip : ""
  ...
  UI_ES = {
    "CHANGE DEX"=>"MUTADEX","Preparing…"=>"Preparando…","Loading comparison index"=>"Preparando índice de comparación",
    "No changes detected"=>"No se detectaron cambios","No changes detected in abilities."=>"No se detectaron cambios en habilidades.","Select Category"=>"Seleccionar categoría","Change Dex"=>"Mutadex",
    ...
    "New Evolutions:"=>"Nuevas evoluciones:","Added Evolution Methods:"=>"Métodos de evolución añadidos:",
    ...
    "{1} evolves via {2}"=>"{1} evoluciona mediante {2}","??? evolves via {1}"=>"??? evoluciona mediante {1}",
    ...
    "No match found in Change Dex."=>"No se encontró ninguna coincidencia en Mutadex.","Type · Class · Power"=>"Tipo · Clase · Potencia",
    ...
    "Open the ChangeDex comparison screen."=>"Abrir la pantalla de comparación de Mutadex.",
  ...
  def self.ui(text, *args)
    ...
    if language_code == "es"
      ...
      raw = "CHANGEDEX" if raw == "CHANGE DEX"
      raw = "ChangeDex" if raw == "Change Dex"
      raw = "No match found in ChangeDex." if raw == "No match found in Change Dex."
    end
  ...
  class Scene
    ...
    def detect_changes
      ...
      evo_rows = proc do |raw|
        CarnekChangeData.parse_evolution_rows(raw)
      end
    ...
    def evolution_method_change_data(species_data, canon)
      empty = { :new => [], :added => [], :changed => [], :canon => [] }
      return empty if !species_data
      ...
      current_list.reject! { |e| e[:method].to_s.downcase == "none" }

      new_list = []
      added_list = []
      changed_list = []
      targets = current_list.map { |e| e[:target_key] }.uniq
      targets.each do |target|
        cur_group = current_list.select { |e| e[:target_key] == target }
        can_group = canon_list.select { |e| e[:target_key] == target }
        if can_group.empty?
          new_list.concat(cur_group)
          next
        end

        used_cur = {}
        used_can = {}
        # First consume every exact canon method. This is crucial when a species
        # keeps its original evolution and merely gains a second way to reach the
        # same target (e.g. Graveler -> Golem by Trade + a new stone method).
        cur_group.each_with_index do |cur, ci|
          idx = can_group.each_index.find do |ki|
            !used_can[ki] && can_group[ki][:method] == cur[:method] && can_group[ki][:param].to_s == cur[:param].to_s
          end
          next if idx.nil?
          used_cur[ci] = true
          used_can[idx] = true
        ...
        # A remaining current method with the same method ID as a remaining canon
        # row is a changed parameter, not an added evolution.
        cur_group.each_with_index do |cur, ci|
          next if used_cur[ci]
          idx = can_group.each_index.find { |ki| !used_can[ki] && can_group[ki][:method] == cur[:method] }
          next if idx.nil?
          used_cur[ci] = true
          used_can[idx] = true
          changed_list << { :species => cur[:species], :from => can_group[idx], :to => cur }
        end

        remaining_cur = cur_group.each_index.reject { |ci| used_cur[ci] }
        remaining_can = can_group.each_index.reject { |ki| used_can[ki] }
        # Pair replacements only while a canon row still exists. Any extra current
        # rows after that are genuinely ADDED METHODS to the same species target.
        pair_count = [remaining_cur.length, remaining_can.length].min
        pair_count.times do |n|
          ci = remaining_cur[n]; ki = remaining_can[n]
          used_cur[ci] = true; used_can[ki] = true
          changed_list << { :species => cur_group[ci][:species], :from => can_group[ki], :to => cur_group[ci] }
        end
        remaining_cur.drop(pair_count).each do |ci|
          added_list << cur_group[ci].merge(:added_method => true)
        end
      end
      return { :new => new_list, :added => added_list, :changed => changed_list, :canon => canon_list }
    end

    def has_evolution_method_changes?(species_data, canon)
      data = evolution_method_change_data(species_data, canon)
      return !data[:new].empty? || !data[:added].empty? || !data[:changed].empty?
    end
    ...
    def show_evolution_method_window(species, form)
      ...
      return if evo_data[:new].empty? && evo_data[:added].empty? && evo_data[:changed].empty?
      lines = []
      if !evo_data[:new].empty?
        lines << VermeilChangeDex.ui("New Evolutions:")
      ...
      if !evo_data[:added].empty?
        lines << "" if !lines.empty?
        lines << VermeilChangeDex.ui("Added Evolution Methods:")
        evo_data[:added].each do |evo|
          evo_name = VermeilChangeDex.localized_species_name(evo[:species])
          method_txt = changedex_evo_method_short_label(evo[:method], evo[:param])
          lines << VermeilChangeDex.ui("{1}: added method {2}", evo_name, method_txt)
        end
      end
  ...
  def self.open
    ...
    begin
      ...
      echoln("[ChangeDex] Runtime refresh skipped: #{e.message}") if defined?(echoln) && ChangeDexConfig.debug_logging?
    end
    scene = Scene.new
    scene.pbStartScene
    scene.pbMain
...
# v0.11.19: keep the JSON overlay completely out of every Essentials PBS
# compiler/writer entry point. The lazy installer below matters for projects
# where plugins are evaluated before Compiler itself is defined.
# v0.11.24+: COMPILER-TRANSPARENT ARCHITECTURE
# ChangeDex is a runtime overlay and must not prepend/alias any Compiler method.
# LA BASE DE SKY and New Animation Editor retain complete ownership of PBS
# compilation. The legacy guard API remains as an inert compatibility shim so
# stale callers cannot reinstall old hooks.
module CarnekChangeDexCompilerGuard
end

module CarnekChangeData
  def self.install_compiler_guard_if_available!
    return false
  end
end
...
# v0.11.15: ChangeDex only prewarms its comparison index here. Translation is
...
if defined?(Game)
  module ChangeDexBootPrewarm
    def initialize(*args, &block)
      ...
      begin
        # Compiler.main has already completed before Game.initialize. Apply the
        # runtime JSON only here, never while PBS/GameData registries are being
        # rebuilt.
        CarnekChangeData.mark_stale!
        CarnekChangeData.apply! if CarnekChangeData.runtime_data_ready?
      rescue StandardError => e
        echoln("[ChangeDex] Runtime overlay boot skipped: #{e.message}") if defined?(echoln) && ChangeDexConfig.debug_logging?
      end
      begin
...
# v0.11.16 Studio provenance/ACL fix: runtime behavior unchanged.

# v0.11.17: recovery-aware JSON loader for Windows/OneDrive write locks.
# v0.11.18: PBS compiler guard prevents runtime JSON overrides from touching half-built GameData.
# v0.11.19: lazy compiler/writer guard + call-stack fallback; fixes false missing-species evolution IDs (e.g. GOLBAT -> CROBAT) in split pokemon*.txt compilation.
# v0.11.20: compatibility fix for New Animation Editor; ChangeDex no longer wraps Compiler.main, preventing recursive main -> main alias loops while retaining lower-level compiler guards.
# v0.11.21: hard compiler-context bypass + legacy [MakerStudio] runtime shutdown; fixes split-PBS validation seeing false missing species such as CROBAT.
# v0.11.22: layered PBS expansion for Species/Move/Ability/Item partial compiler calls.
# v0.11.23: safe GameData bootstrap installs Compiler guards before compile_pokemon without wrapping main; fixes guards silently never installing after the New Animation Editor recursion fix.

# v0.11.26: fixes deployment of the legacy shutdown stub to the real
# Maker Studio plugin path (Plugins/MakerStudio/001_ChangeDex.rb). Runtime
# compiler behavior remains identical to v0.11.24: ChangeDex is absent from
# PBS compilation and applies JSON only after Compiler finishes.
