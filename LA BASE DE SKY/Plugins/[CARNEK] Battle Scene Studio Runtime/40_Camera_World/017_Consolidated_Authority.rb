...
module BSS084SceneAuthority
  ...
  def bss083_nds_world_snapshot(foes)
    ...
    (@sprites || {}).each do |key, sp|
      ...
      should_hide = (k =~ /^(battle_bg|battle_bg2|base_\d+|pokemon_|shadow_|dataBox_|trainer_|party|cmdBar|command|fight|target|message|enhanced|info_icon|ball_icon|leftarrow|rightarrow|boss|ability|itemWindow)/i) && !(k =~ /^(bss_|custom_)/i) || (k =~ /_outline\d+$/i)
