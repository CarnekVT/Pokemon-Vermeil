...
module BSS083
  # ---------------------------------------------------------------------------
  # Camera approach utilities – adds custom zoom levels for shots.
  # ---------------------------------------------------------------------------
  module Approach
    # Returns a camera vector with a custom zoom level.
    # shot: symbolic shot (:player, :enemy, :wide, etc.)
    # zoom: float multiplier (1.0 = default)
    def self.approach_vector(shot, battle, zoom: 1.0)
      vec = BSS083.camera_shot(shot, battle)
      # The zoom component is the 6th element (index 5) in the vector.
      vec[5] = zoom.to_f if vec.is_a?(Array) && vec.size >= 6
      vec
    end
  end
...
module BSS083SceneCameraAuthority
  ...
  def bss070_ebdx_camera_enter(mode)
    ...
    @bss070_ebdx_camera_mode = @bss083_camera_state
    if Settings.camera_approach_enabled?
      zoom = Settings.zoom_level(:default)
      shot = @bss083_camera_state
      @vector.inc = 0.028
      @vector.set(BSS083::Approach.approach_vector(shot, @battle, zoom: zoom))
    end
    @bss083_idle_due ||= BSS070EBDXCore::BATTLE_MOTION_TIMER.to_i * BSS070EBDXCore.frame_rate.to_i
    nil
  end
  def bss070_ebdx_camera_leave
    ...
    if Settings.camera_approach_enabled?
      ...
      @bss070_ebdx_camera_mode = Settings.dynamic_camera_enabled? ? BSS083::DYNAMIC_STATE : :idle
    end
    @bss083_idle_due ||= BSS070EBDXCore::BATTLE_MOTION_TIMER.to_i * BSS070EBDXCore.frame_rate.to_i
    nil
  end
  ...
  def bss070_ebdx_tick(advance_camera=true, align=true)
    ...
    frozen = @bss083_camera_freeze.to_i > 0 || @bss070_ebdx_bas_frame || @bss070_ebdx_suspend_depth.to_i > 0
    ...
    @bss083_idle_due ||= (2 * BSS070EBDXCore.frame_rate.to_i)
    state = (@bss083_camera_state || :idle).to_sym
    if state == :settle && @vector.finished?
      @bss083_camera_state = :idle
      state = :idle
    end
    return unless state == :idle || state == :command || state == :fight

    @bss083_idle_due -= 1
    if @bss083_idle_due <= 0 && @vector.finished?
      is_command_phase = (@bss070_ebdx_camera_mode == :command || @bss070_ebdx_camera_mode == :fight)
      if is_command_phase
        roll = rand(100)
        if roll < 30
          @bss083_idle_side = :player
        elsif roll < 60
          @bss083_idle_side = :opponent
        else
          @bss083_idle_side = (@bss083_idle_side == :idle_left ? :idle_right : :idle_left)
        end
        @vector.inc = 0.008
        @vector.set(BSS083.camera_shot(@bss083_idle_side, @battle))
        @bss083_idle_due = (3 * BSS070EBDXCore.frame_rate.to_i)
      else
        # If the user disabled sliding in Carnek Project Settings, we simulate
        # the lack of intro-slide by keeping the camera rigidly locked to MAIN 
        # instead of letting EBDX pan across the opponent and player.
        disable_slide = defined?(Settings) && Settings.const_defined?(:DISABLE_SLIDING_BACKGROUND) && Settings::DISABLE_SLIDING_BACKGROUND
        if disable_slide
          @vector.inc = 0.015
          @vector.set(BSS083.camera_shot(:main, @battle))
        end
