...
module BSS085
  ...
  # Null-object proxy returned by battlerSprite when the real sprite is nil.
  # Responds to vanishMode, substitute, prepare_mosaic= etc. with safe defaults
  # so that DBK/Animated Pokémon System code never crashes on nil:NilClass.
  class NullBattlerSprite
    def vanishMode;       0;     end
    def substitute;       false; end
    def prepare_mosaic;   false; end
    def prepare_mosaic=(v); end
    def visible;          false; end
    def visible=(v);      end
    def opacity;          0;     end
    def opacity=(v);      end
    def disposed?;        true;  end
    def respond_to_missing?(m, *); true; end
    def method_missing(m, *args); nil; end
  end
  NULL_SPRITE = NullBattlerSprite.new
end

# Patch Battle::Battler#battlerSprite to never return nil.
# DBK defines battlerSprite as: @battle.scene.sprites["pokemon_#{self.index}"]
# which can return nil before sprites are created or after disposal.
if defined?(Battle::Battler)
  class Battle::Battler
    alias _bss085_orig_battlerSprite battlerSprite if method_defined?(:battlerSprite)
    def battlerSprite
      sp = _bss085_orig_battlerSprite rescue nil
      sp || BSS085::NULL_SPRITE
    end
  end
end
...
module BSS085SceneAuthority
  ...
  def bss085_enhanced_ui_layers
    ...
    main=@sprites["enhancedUI"] rescue nil; main.z=12000 if main && !(main.disposed? rescue true) && main.respond_to?(:z=)
    prompt=@sprites["enhancedUIPrompts"] rescue nil; prompt.z=12060 if prompt && !(prompt.disposed? rescue true) && prompt.respond_to?(:z=)
    ["leftarrow","rightarrow"].each{|k| sp=@sprites[k] rescue nil; sp.z=12100 if sp && !(sp.disposed? rescue true) && sp.respond_to?(:z=)}
    @sprites.each do |k,sp|
      next if !sp || (sp.disposed? rescue true) || !sp.respond_to?(:z=); s=k.to_s
      if s =~ /^(info_icon|ball_icon).*_outline\d+$/; sp.z=12140
      elsif s =~ /^(info_icon|ball_icon)\d+$/; sp.z=12141
      end
    end
  rescue
  end
  ...
  # Save and restore battler geometry so that Y positions remain stable.
    ...
    def createProcesses
      ...
      @battle.battlers.each do |b|
        ...
        if b.index==@idx_sos
          ...
          ebdx_active = (defined?(BSS078) && BSS078.ebdx_room?(@sprites))
          obj=addSprite(bat,PictureOrigin::BOTTOM); obj.setZ(0,nz) if obj.respond_to?(:setZ)
          obj.moveXY(0,duration,nx,bat.y) unless ebdx_active
          if sha; sx,_sy,sz=bss_sos_shadow_position(b,side_size,sha); sh=addSprite(sha,PictureOrigin::CENTER); sh.setZ(0,sz) if sh.respond_to?(:setZ); sh.moveXY(0,duration,sx,sha.y) unless ebdx_active; end
