...
module BSS090
  ...
  # Added guard against cyclic structures and non‑Hash/Array recursion.
  require 'set' unless defined?(Set)
  def deep_symbolize(obj, visited = nil, depth = 0)
    visited ||= Set.new
    # If the object is a Hash with all Symbol keys, assume it's already symbolized.
    return obj if obj.is_a?(Hash) && obj.keys.all? { |k| k.is_a?(Symbol) }
    # Guard against excessively deep recursion.
    return obj if depth > 50
    ...
    if obj.is_a?(Hash)
      obj.each_with_object({}) do |(k, v), result|
        ...
        result[key] = deep_symbolize(v, visited, depth + 1)
      end
    else # Array
      obj.map { |e| deep_symbolize(e, visited, depth + 1) }
    end
  end
