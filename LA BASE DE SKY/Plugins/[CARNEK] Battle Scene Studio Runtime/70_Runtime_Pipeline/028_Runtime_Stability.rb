...
module BSS100EnhancedZ
  def bss100_enhanced_z_only
    ...
    main.z=999300 if main && !(main.disposed? rescue true) && main.respond_to?(:z=)
    prompt.z=999360 if prompt && !(prompt.disposed? rescue true) && prompt.respond_to?(:z=)
    ["leftarrow","rightarrow"].each do |k|
      sp=@sprites[k] rescue nil
      sp.z=999410 if sp && !(sp.disposed? rescue true) && sp.respond_to?(:z=)
    end
    ...
    @sprites.each do |k,sp|
      ...
      sp.z=(parent && parent.respond_to?(:z) ? parent.z.to_i-1 : 999441)
    end
  rescue
  end
