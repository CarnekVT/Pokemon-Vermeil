...
module BSS064
  ...
  class << self
    ...
    def write_status(state, extra = {})
      ...
      require 'fileutils'
      dir = File.expand_path('Data/BattleSceneStudio')
      FileUtils.mkdir_p(dir) unless Dir.exist?(dir)
      raw = json_generate(payload)
      path = File.join(dir, 'runtime_status.json')
      # Use File.write which handles binary mode and ensures proper encoding on Windows
      File.write(path, raw, mode: 'wb')
      true
