...
class Battle
  ...
  def bss_call_for_help(caller, guaranteed=false, acts_this_round=false, show_intro=true)
    return false if !caller || (!@bss_sos_enabled && !guaranteed)
    @bss_sos_enabled = true if guaranteed
    begin
  ...
  def createProcesses
    ...
    @battle.battlers.each do |b|
      ...
      if b.index == @idx_sos
        ...
        side_size = bat.respond_to?(:sideSize) && bat.sideSize ? bat.sideSize : @battle.pbSideSize(b.index)
        new_x, _new_y, new_z = bss_sos_battler_position(b, side_size, bat)
        obj = addSprite(bat, PictureOrigin::BOTTOM)
        obj.setZ(delay, new_z) if obj.respond_to?(:setZ)
        obj.moveXY(delay, 4, new_x, bat.y)
        if sha
          shadow_size = sha.respond_to?(:sideSize) && sha.sideSize ? sha.sideSize : side_size
          sx, _sy, sz = bss_sos_shadow_position(b, shadow_size, sha)
          sh = addSprite(sha, PictureOrigin::CENTER)
          sh.setZ(delay, sz) if sh.respond_to?(:setZ)
          sh.moveXY(delay, 4, sx, sha.y)
        end
        if boxsp
...
if defined?(ItemHandlers)
  ItemHandlers::CanUseInBattle.add(:ADRENALINEORB, proc { |_item, _pokemon, _battler, _move, _firstAction, battle, _scene, _showMessages|
    next false if !battle || !(battle.wildBattle? rescue false)
    caller = battle.battlers.find { |b| b && b.opposes? && !b.fainted? }
    next false if !caller
    max_allies = battle.respond_to?(:bss_sos_max_live_same_side) ? battle.bss_sos_max_live_same_side : 2
    live_foes = battle.battlers.count { |b| b && b.opposes? && !b.fainted? }
    next false if live_foes >= max_allies
    next true
  })
  ...
  ItemHandlers::UseInBattle.add(:ADRENALINEORB, proc { |item, battler, battle|
    ...
    max_allies = battle.respond_to?(:bss_sos_max_live_same_side) ? battle.bss_sos_max_live_same_side : 2
    live_foes = battle.battlers.count { |b| b && b.opposes? && !b.fainted? }
    if !caller || live_foes >= max_allies
      battle.pbDisplay(_INTL("¡Pero no tuvo efecto!"))
      battle.pbReturnUnusedItemToBag(item, battler.index) rescue nil
      next
    end
    # Ensure SOS is enabled for this battle even if global requirements were not met
    battle.bss_sos_enabled = true
    if !battle.bss_sos_config.is_a?(Hash) || battle.bss_sos_config.empty?
