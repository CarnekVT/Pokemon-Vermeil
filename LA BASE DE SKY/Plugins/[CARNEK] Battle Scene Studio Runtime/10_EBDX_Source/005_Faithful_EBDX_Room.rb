...
class BSS070EBDXRoom
  ...
  def initialize(viewport, scene, data)
    ...
    @focused = true
    @queued = nil
    # Deep symbolize the data hash so string keys from JSON map to :bitmap, :x, etc.
    @data = {}
    (data || {}).each do |k, v|
      if v.is_a?(Hash)
        new_v = {}
        v.each { |vk, vv| new_v[vk.to_sym] = vv }
        @data[k.to_s] = new_v
      else
        @data[k.to_s] = v
      end
  ...
  def refresh(*args)
    ...
    if @sprites["bg"].bitmap
      ...
      if @baseBmp
        # The base should always anchor flush to the logical bottom of the view,
        # which corresponds to exactly where the horizon ends, irrespective of
        # how much extra padding the wideWorld bitmap has underneath.
        logical_h = 308.0
        sy = @scene.vector.spoof(@defaultvector)[1]
        oy = sy / 1.5 + 16
        
        if @bss_wide_world_origin
          oy += @bss_wide_world_origin[1]
        end
        
        dist_from_bottom_of_screen = (logical_h / 2.0)
        dist_from_top_of_screen = (logical_h / 2.0)
        
        # Center of camera is oy. Bottom of screen is oy + dist_from_bottom_of_screen.
        screen_bottom_y = oy + dist_from_bottom_of_screen
        
        # We want the bottom of the base to be at screen_bottom_y.
