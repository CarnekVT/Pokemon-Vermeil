  ...
  module ProgressiveZoom
    ...
    class << self
      ...
      def update
        ...
        # ponytail: pocos tramos por mapa; indexar por celda si crece.
        ...
        weight = if axis <= profile[:middle_axis]
                   (axis - profile[:start_axis]) /
                     (profile[:middle_axis] - profile[:start_axis])
                 else
                   (profile[:end_axis] - axis) /
                     (profile[:end_axis] - profile[:middle_axis])
                 end
        weight = weight.clamp(0.0, 1.0)
        weight = weight * weight * (3.0 - 2.0 * weight)
        base = Mode7.camera_zoom.to_f
        Mode7.zoom_effect_override = base + (profile[:middle_zoom] - base) * weight
      rescue Exception
        Mode7.zoom_effect_override = nil
      end
      ...
      def load_profiles(map_id)
        data = JSON.parse(File.read(FILE_PATH))
        maps = data.is_a?(Hash) ? data["maps"] : nil
        map = maps.is_a?(Hash) ? maps[map_id.to_s] : nil
        profiles = map.is_a?(Hash) ? map["profiles"] : nil
        return [] if !profiles.is_a?(Hash)
      ...
      def normalize_profile(profile)
...

        {
          ...
          cells: cells.each_with_object({}) { |cell, index| index[cell.to_s] = true }
        }
      end
