class FaintedBitmap
  ...
  def width; @frames[0]&.width || 0; end
  def height; @frames[0]&.height || 0; end
  def length; @frames.length; end

  def update
    return if @frames.length <= 1 || @speed <= 0
    delay = ((@speed / 2.0) * 60).round / 1000.0
    return if System.uptime - @last_update < delay
  ...
  def load_frames(pkmn, back)
    ...
    cdir = "Graphics/Pokemon/#{subdir}/Fainted/"
    base = "#{cdir}#{species}"
    file = nil
    %w[.png .gif].each do |e|
