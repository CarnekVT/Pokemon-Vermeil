...
class PokemonPartyBlankPanel < Sprite
  ...
  def initialize(_pokemon, index, viewport = nil)
    super(viewport)
    self.x = 55+ (index % 2) * Graphics.width / 2.3
    self.y = (96 * (index / 2)) + 30
...
class PokemonPartyPanel < Sprite
  ...
  def initialize(pokemon, index, viewport = nil)
    ...
    @refreshing = true
    self.x = 55+(index % 2) * Graphics.width / 2.3 #Aquí mueves las coordenadas del bloque de cada bicho(nivel, nombre, género...)
    self.y = (96 * (index / 2))+30
...
def draw_gender
  ...
  gender_bitmap = Bitmap.new("Graphics/UI/Party/icon_genders")

  icon_w = gender_bitmap.width / 2   # 22 px

  src_rect = Rect.new(
    @pokemon.male? ? 0 : icon_w,  # X origen (♂ o ♀)
    0,                            # Y origen
    icon_w,                       # ancho
    gender_bitmap.height          # alto (22)
  )

  # Coordenadas del texto original (224,22) + compensación vertical
  bmp.blt(224, 16, gender_bitmap, src_rect)
end
