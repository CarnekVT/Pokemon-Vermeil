...
class Battle::Scene::PokemonDataBox
  ...
  def initializeOtherGraphics(viewport)
    if @style
      ...
      color_count = defined?(HP_COLOR_COUNT) ? HP_COLOR_COUNT : 3
      @hpBar.src_rect.height = @hpBarBitmap.height / color_count
      @sprites["hpBar"] = @hpBar
  ...
  def update_style
    ...
    if sideSize > @style.max_side_size && @style.id != :Basic
      ...
      color_count = defined?(HP_COLOR_COUNT) ? HP_COLOR_COUNT : 3
      @hpBar.src_rect.height = @hpBarBitmap.height / color_count if @hpBar
