...
class Pokemon
  ...
  def calcHP(base, level, iv, ev)
    return 1 if base == 1
    iv = ev = 0 if Settings::DISABLE_IVS_AND_EVS || (defined?(MinimalGrinding) && MinimalGrinding.on?)
...
class Battle::Battler
  ...
  def pbCanConfuse?(*args)
    ...
    if immunities.include?(:ALLSTATUS) || immunities.include?(:CONFUSED)
      @battle.pbDisplay(_INTL("¡{1} es completamente inmune a la confusión!", pbThis)) if args[1]
      return false
    end
  ...
  def pbCanAttract?(user, showMessages = true)
    ...
    if immunities.include?(:ALLSTATUS) || immunities.include?(:ATTRACT)
      @battle.pbDisplay(_INTL("¡{1} es completamente inmune al enamoramiento!", pbThis)) if showMessages
      return false
    end
...
class Battle::Move  
  ...
  def pbMoveFailedAromaVeil?(user, target, showMessage = true)
    if target.pokemon.immunities.include?(:DISABLE)
      @battle.pbDisplay(_INTL("¡{1} es completamente inmune a los efectos que pueden deshabilitar sus movimientos!", target.pbThis)) if showMessage
      return true
    end
