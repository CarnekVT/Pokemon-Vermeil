...
#===============================================================================
class Battle::Move
  # Helper to call trigger methods that may accept 4 or 5 args.
  def self.trigger_with_variable_arity(mod, method_sym, *args)
    return nil unless mod.respond_to?(method_sym)
    m = mod.method(method_sym)
    params = m.parameters.reject { |type, _| type == :block }
    if params.any? { |type, _| type == :rest } || params.length >= args.length
      return mod.send(method_sym, *args)
    end
    return mod.send(method_sym, *args[0, params.length])
  end
  ...
  def crit_stage_bonuses(user)
    ...
    if defined?(user.criticalHitRate)
      bonus += user&.criticalHitRate
    else
  ...
  def pbIsCritical?(user, target)
    ...
    if c >= 0 && user.abilityActive?
      c = Battle::Move.trigger_with_variable_arity(Battle::AbilityEffects,
                                                   :triggerCriticalCalcFromUser,
                                                   user.ability, user, target, c, self)
    end
    if c >= 0 && target.abilityActive? && !@battle.moldBreaker
      c = Battle::Move.trigger_with_variable_arity(Battle::AbilityEffects,
                                                   :triggerCriticalCalcFromTarget,
                                                   target.ability, user, target, c, self)
    end
    if c >= 0 && user.itemActive?
      c = Battle::Move.trigger_with_variable_arity(Battle::ItemEffects,
                                                   :triggerCriticalCalcFromUser,
                                                   user.item, user, target, c, self)
    end
    if c >= 0 && target.itemActive?
      c = Battle::Move.trigger_with_variable_arity(Battle::ItemEffects,
                                                   :triggerCriticalCalcFromTarget,
                                                   target.item, user, target, c, self)
    end
    return false if c < 0
...
class Battle::AI::AIMove
  ...
  def rough_critical_hit_stage
    ...
    crit_stage = 0
    if user.ability_active?
      crit_stage = Battle::Move.trigger_with_variable_arity(Battle::AbilityEffects,
                                                            :triggerCriticalCalcFromUser,
                                                            user_battler.ability, user_battler, target_battler, crit_stage, self.move)
      return -1 if crit_stage < 0
    end
    if !@ai.battle.moldBreaker && target.ability_active?
      crit_stage = Battle::Move.trigger_with_variable_arity(Battle::AbilityEffects,
                                                            :triggerCriticalCalcFromTarget,
                                                            target_battler.ability, user_battler, target_battler, crit_stage, self.move)
      return -1 if crit_stage < 0
    end
    if user.item_active?
      crit_stage = Battle::Move.trigger_with_variable_arity(Battle::ItemEffects,
                                                            :triggerCriticalCalcFromUser,
                                                            user_battler.item, user_battler, target_battler, crit_stage, self.move)
      return -1 if crit_stage < 0
    end
    if target.item_active?
      crit_stage = Battle::Move.trigger_with_variable_arity(Battle::ItemEffects,
                                                            :triggerCriticalCalcFromTarget,
                                                            target_battler.item, user_battler, target_battler, crit_stage, self.move)
      return -1 if crit_stage < 0
    end
