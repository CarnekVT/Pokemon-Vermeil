...
module Compiler
  ...
  def edit_and_rewrite_pbs_file_text(filename)
    ...
    if changed
      if @plugin_change
        msg = "Cambios hechos en el archivo #{filename} por el plugin [#{@plugin_change}]."
      else
        msg = "Cambios hechos en el archivo #{filename}."
      end
      ...
      text_files.each_key do |data_type|
        ...
        text_files[data_type][1].each do |file_name|
          ...
          File.open(path, "rb") do |f|
            f.each_line do |line|
              if line == "### Aplica los cambios manteniendo SHIFT mientras se compila. ###\r\n"
          ...
          if !data_hash.empty?
            ...
            edit_and_rewrite_pbs_file_text("#{file_name}") do |line|
              ...
              line == "### Aplica los cambios manteniendo SHIFT mientras se compila. ###\r\n"
              if line[/^\s*\[\s*(.*)\s*\]\s*$/]
          ...
          if @plugin_change
            ...
            next line.gsub!("### Los cambios se aplicarán automáticamente. ###", 
                            "### Aplica los cambios manteniendo SHIFT mientras se compila. ###")
            end
          end
        end
      end
