#===============================================================================
# 003_Scene.rb - Interfaz Gráfica de Usuario (Frontend)
#===============================================================================
module VermeilChangeDex
  class Scene
    include VermeilChangeDex
    def initialize
      @mode = :grid; @index = 0; @viewport = Viewport.new(0, 0, SCREEN_W, SCREEN_H)
      @viewport.z = 99999; @sprites = {}; @entries = []
      @typebitmap = AnimatedBitmap.new(_INTL("Graphics/UI/types")) rescue nil
    ...
    def species_stats_array(species_data)
      ...
      return [bs[:HP].to_i, bs[:ATTACK].to_i, bs[:DEFENSE].to_i, bs[:SPEED].to_i, bs[:SPECIAL_ATTACK].to_i, bs[:SPECIAL_DEFENSE].to_i]
    end
    ...
    def force_visible_hidden_variant?(species, form)
      return false if form <= 0
      return false if !VermeilChangeDex::FORCE_VISIBLE_HIDDEN_VARIANT_SPECIES.include?(species)
    ...
    def apply_entry_filters
      ...
      if @category == :move_changes || @category == :new_moves
        ...
        if @entry_exclusive_filter == :globalized
          list.select! do |e|
            next false if VermeilChangeDex::EXCLUSIVE_GLOBALIZED_FILTER_EXCLUDED_IDS.include?(e[:id])
    ...
    def ability_boost_popup_lines(ability_id, bmp)
      ...
      return wrap_colored_tokens(bmp, tokens, 540)
    end
    ...
    def draw_grid_interface
      ...
      @sprites["overlay"].bitmap.fill_rect(0, 436, SCREEN_W, 44, Color.new(0, 0, 0, 236))
      @sprites["overlay"].bitmap.fill_rect(0, 434, SCREEN_W, 2, COLOR_HIGHLIGHT)
      @sprites["overlay"].bitmap.fill_rect((SCREEN_W / 2) - 112, 438, 224, 38, Color.new(12, 16, 24, 210))
      @evo_mark_base_y = {}
      @variant_mark_base_y = {}
      @sprites.keys.each do |k|
    ...
    def draw_list_interface
      ...
      bmp.fill_rect(0, 436, SCREEN_W, 44, Color.new(0, 0, 0, 236))
      bmp.fill_rect(0, 434, SCREEN_W, 2, COLOR_HIGHLIGHT)
      ...
      text_positions = [[label, SCREEN_W / 2, 446, :center, COLOR_TEXT_MAIN, Color.new(0,0,0,160)],
                        [_INTL("{1}: Search", @action_key_name), 8, 446, :left, COLOR_TEXT_GRAY, Color.new(0,0,0,160)]]
      text_positions << [right_hint, SCREEN_W - 8, 446, :right, COLOR_TEXT_GRAY, Color.new(0,0,0,160)] if right_hint && !right_hint.empty?
    ...
    def hidden_variant_icon_frame(species, form)
      ...
      s_data = GameData::Species.get_species_form(species, form)
      return nil if !s_data.form_name.to_s.strip.empty?
    ...
    def detail_action_options
      ...
      if pokemon_category?
        ...
        if VermeilChangeDex::SHOW_DETAIL_VARIANT_ACTION
    ...
    def refresh_cursor
      ...
      bmp = @sprites["overlay"].bitmap
      bmp.clear_rect(0, 434, SCREEN_W, 46)
      bmp.fill_rect(0, 436, SCREEN_W, 44, Color.new(0, 0, 0, 236))
      bmp.fill_rect(0, 434, SCREEN_W, 2, COLOR_HIGHLIGHT)
      if @entries.empty?
        pbDrawTextPositions(bmp, [["No matches for current filter.", SCREEN_W / 2, 446, :center, COLOR_TEXT_GRAY, Color.new(0,0,0,160)],
                                  [_INTL("{1}: Search", @action_key_name), 8, 446, :left, COLOR_TEXT_GRAY, Color.new(0,0,0,160)],
                                  [_INTL("{1}: Filter", @filter_key_name), SCREEN_W - 8, 446, :right, COLOR_TEXT_GRAY, Color.new(0,0,0,160)]])
        return
      end
      ...
      pbDrawTextPositions(bmp, [[name, SCREEN_W/2, 446, :center, COLOR_TEXT_MAIN, Color.new(0,0,0,160)],
                                [_INTL("{1}: Search", @action_key_name), 8, 446, :left, COLOR_TEXT_GRAY, Color.new(0,0,0,160)],
                                [_INTL("{1}: Filter", @filter_key_name), SCREEN_W - 8, 446, :right, COLOR_TEXT_GRAY, Color.new(0,0,0,160)]])
      draw_category_panel
    end
    ...
    def draw_entry_detail(entry)
      ...
      draw_panel(bmp, 10, 52, 620, 166)
      draw_panel(bmp, 10, 228, 620, 146)
      y = 56
      if @category == :move_changes || @category == :new_moves
        ...
        draw_wrapped_text(bmp, desc, 20, y + 58, 590, COLOR_TEXT_GRAY, 22, 3)
      else
        a_data = GameData::Ability.try_get(entry[:id])
        return if !a_data
        draw_wrapped_text(bmp, a_data.description.to_s, 20, y, 590, COLOR_TEXT_GRAY, 22, 5)
      end
      draw_carrier_icon_window(bmp, entry)
      if @category == :move_changes && !move_change_lines(entry).empty?
        pbDrawTextPositions(bmp, [[_INTL("{1}: Before/After", @filter_key_name), SCREEN_W - 20, 448, :right, COLOR_TEXT_GRAY, Color.new(0,0,0,120)]])
      end
      if (@category == :ability_changes || @category == :new_abilities)
        if !ability_before_after_lines(entry, @sprites["overlay"].bitmap).empty?
          ...
          pbDrawTextPositions(bmp, [[_INTL("{1}: Boosted Moves", @filter_key_name), SCREEN_W - 20, 448, :right, COLOR_TEXT_GRAY, Color.new(0,0,0,120)]])
        end
      end
      if !pokemon_category?
    ...
    def draw_carrier_icon_window(bmp, entry)
      ...
      if added.empty?
        ...
        draw_wrapped_text(bmp, compact_user_list_text(added, 8), 20, 268, 590, COLOR_MOVES_NEW, 22, 2)
      end
      pbDrawTextPositions(bmp, [["All Carriers: #{users.length}", 620, 244, :right, COLOR_TEXT_GRAY]])
    end
    ...
    def draw_detail_view(species, form)
      ...
      draw_panel(bmp, 10, 52, 220, 200)
      draw_panel(bmp, 240, 52, 390, 200)
      ...
      @sprites["big_icon"].setOffset(PictureOrigin::CENTER); @sprites["big_icon"].x, @sprites["big_icon"].y = LEFT_PANEL_CENTER_X, 130
      ...
      c_types = (canon[:types] || []).to_a; v_types = (vermeil.types || []).to_a; type_y = 166; type_center_x = LEFT_PANEL_CENTER_X
      if c_types.sort != v_types.sort && !c_types.empty?
        draw_type_icons(bmp, c_types, type_center_x, type_y - 20, 150)
        pbDrawTextPositions(bmp, [["New Typing", type_center_x, type_y - 2, :center, COLOR_TEXT_MAIN, Color.new(0,0,0,100)]])
        draw_type_icons(bmp, v_types, type_center_x, type_y + 20)
      else
        draw_type_icons(bmp, v_types, type_center_x, type_y)
      end

      v_stats = species_stats_array(vermeil)
      c_stats = canon[:stats]
      has_new_evos = !gained_evolution_species(vermeil, canon).empty?
      x_base = has_new_evos ? 260 : 310
      y_base = 100
      ...
      abilities_panel_y = 264
      draw_panel(bmp, 10, abilities_panel_y, 620, 74)
      ...
      if display_entries.empty?
        lines = draw_wrapped_text(bmp, "No changes detected in abilities.", 20, abilities_text_y, 590, COLOR_TEXT_GRAY, 22, 2)
        abilities_end_y = abilities_text_y + (lines * 22)
      else
        lines = draw_colored_ability_entries(bmp, display_entries, 20, abilities_text_y, 590, 22, 2)
        abilities_end_y = abilities_text_y + (lines * 22)
      end
      ...
      moves_header_y = [344, abilities_end_y + 6].max
      draw_panel(bmp, 10, moves_header_y - 6, 620, 132)
    ...
    def draw_move_change_section(bmp, level_list, tutor_list, header_y = 296)
      ...
      y += 22
      if combined.empty?
        draw_wrapped_text(bmp, "No moves added to this Pokémon.", 20, y, 590, COLOR_TEXT_GRAY, 22, 2)
      else
        full_text = combined.join(", ")
        draw_wrapped_text(bmp, full_text, 20, y, 590, COLOR_MOVES_NEW, 22, 2)
      end
    end
