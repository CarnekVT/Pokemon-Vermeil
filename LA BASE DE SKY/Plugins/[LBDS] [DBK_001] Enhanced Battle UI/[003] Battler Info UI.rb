...
class Battle::Scene
  ...
  def pbUpdateBattlerInfo(battler, effects, idxEffect = 0)
    ...
    pbAddStatsDisplay(xpos - 25, ypos, battler, imagePos, textPos)
    ...
    [[:ATTACK,          _INTL("Ataque")],
     ...
     _INTL("Críticos")
      ...
      if stat.is_a?(Array)
        ...
        if defined?(battler.stat_multiplier_from_stage)
          # Calculate and display stat multiplier.
          mult = battler.stat_multiplier_from_stage(stat[0])
          mult_text = sprintf("x%.1f", mult)
          mult_color = if stage > 0 then Color.new(136, 96, 72)
                      elsif stage < 0 then Color.new(64, 120, 152)
                      else SHADOW_LIGHT
                      end
          textPos.push([mult_text, xpos + 260, ypos + 138 + (i * 24), :right, BASE_LIGHT, mult_color])
        end
      else
      ...
      if stage != 0
        ...
        stage.abs.times do |t|
          imagePos.push([@path + "info_stats", xpos + 112 + (t * 19), ypos + 136 + (i * 24), arrow, 0, 18, 18])
        end
      end
    end
  end
  ...
  def pbGetDisplayEffects(battler)
    ...
    # Special states.
    if battler.mega?
      desc = _INTL("El Pokémon está en su forma Mega Evolucionada.")
      display_effects.push([_INTL("Mega Evolución"), "--", desc])
    elsif battler.primal?
      desc = _INTL("El Pokémon está en su forma Primigenia.")
      display_effects.push([_INTL("Reversión Primigenia"), "--", desc])
    elsif battler.ultra?
      desc = _INTL("El Pokémon está en su forma Ultra.")
      display_effects.push([_INTL("Ultra"), "--", desc])
    ...
    if (battler.hasActiveAbility?(:PROTOSYNTHESIS) || battler.hasActiveAbility?(:QUARKDRIVE)) && battler.effects[PBEffects::ProtosynthesisStat]
      name = GameData::Ability.get(battler.ability).name
      percent = battler.effects[PBEffects::ProtosynthesisStat] == :SPEED ? "50%" : "30%"
      stat_name = GameData::Stat.get(battler.effects[PBEffects::ProtosynthesisStat]).name
      ...
      key_hash.each do |type, effects|
        effects.each do |effect|
          ...
          when :AuroraVeil
            ...
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("El Pokémon recibe la mitad de daño de los ataques físicos y especiales.")
          #---------------------------------------------------------------------
          when :Reflect
            name = GameData::Move.get(:REFLECT).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("El Pokémon recibe la mitad de daño de los ataques físicos.")
          #---------------------------------------------------------------------
          when :LightScreen
            name = GameData::Move.get(:LIGHTSCREEN).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("El Pokémon recibe la mitad de daño de los ataques especiales.")
          #---------------------------------------------------------------------
          when :Safeguard
          ...
          when :LuckyChant
            ...
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("El Pokémon no puede recibir golpes críticos.")
          #---------------------------------------------------------------------
          when :Gravity
