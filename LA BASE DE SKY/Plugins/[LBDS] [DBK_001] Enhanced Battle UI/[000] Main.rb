...
class Battle::Scene::EnhancedUIPrompt < Sprite
  ...
  def refresh
    return if !@battler
    offset = 0
    unless Settings::PROMPT_TEXT_INCLUDED_IN_GRAPHICS
      textPos = [
    ...
    when Battle::Scene::FIGHT_BOX
      ...
      if @battle.pbCanUsePokeBall?(@battler)
        ...
        @bgSprite.src_rect.height = 28
        textPos.delete_at(1) unless Settings::PROMPT_TEXT_INCLUDED_IN_GRAPHICS
        offset = 24
      end
    end
    self.y = 236 + offset
    self.bitmap.clear
    pbDrawTextPositions(self.bitmap, textPos) unless Settings::PROMPT_TEXT_INCLUDED_IN_GRAPHICS
  end
