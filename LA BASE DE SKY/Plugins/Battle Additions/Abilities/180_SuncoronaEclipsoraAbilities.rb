...
module Battle::AbilityEffects
  module VermeilLightMoves
    def self.light_move_id?(move_or_id)
      return VermeilMoveClasses.light_move?(move_or_id)
    end
  end
  ...
  # 4) Self is immune to Water (evaporation). Use MoveImmunity rather than a
  # zero damage multiplier, because the normal damage pipeline clamps damage.
  MoveImmunity.add(:BLESSEDCORONA,
    proc { |ability, user, target, move, type, battle, show_message|
      ...
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          battle.pbDisplay(_INTL("{1}'s {2} evaporated the Water-type attack!",
...
class Battle::Scene
  ...
  def pbUpdateExecutionerShadowFog
    ...
    @exec_shadow_fog_shift_x += -10 * VermeilBattleAdditions.graphics_delta
    @exec_shadow_fog_shift_y += 4 * VermeilBattleAdditions.graphics_delta
