#===============================================================================
# * Item Ball Printer - by FL (Credits will be appreciated)
#===============================================================================
#
...
#== HOW TO USE =================================================================
#
# The options will appear at debug on pause menu (on "Item options..." 
# submenu). 
#
#=== NOTES =====================================================================
#
...
module ItemBallPrinter
  ...
  def self.generate_txt(type = :item_balls)
    string = "Created on "+Time.now.strftime("%Y-%m-%d %H:%M:%S")
  ...
  def self.on_change_hidden_items_press
    self.include_hidden_items = pbShowCommands(
      nil,
      [_INTL("Yes"), _INTL("No"), _INTL("Only hidden items")],
...
MenuHandlers.add(:debug_menu, :item_ball_printer, {
  "parent"      => :items_menu,
  "name"        => _INTL("Print item events"),
  "description" => _INTL("Prints item events on the map."),
})

MenuHandlers.add(:debug_menu, :item_ball_print_current_map, {
  "parent"      => :item_ball_printer,
  "name"        => _INTL("Print items on current map"),
  "description" => _INTL("Prints item events on the current map."),
  "effect"      => proc{
    receive_items = pbConfirmMessage(_INTL("Do you also want to print items given by NPCs? (pbReceiveItem)"))
...
MenuHandlers.add(:debug_menu, :item_ball_print_all, {
  "parent"      => :item_ball_printer,
  "name"        => _INTL("Print all maps"),
  "description" => _INTL("Prints all maps in {1}.", ItemBallPrinter.file_full_name),
  "effect"      => proc{
    msgwindow = pbCreateMessageWindow
    if FileTest.exist?(ItemBallPrinter.file_full_name) && !pbConfirmMessageSerious(
       _INTL("{1} already exists. Do you want to overwrite it?",ItemBallPrinter.file_full_name)
    )
    ...
    pbMessageDisplay(msgwindow,_INTL("Printed!"))
    pbDisposeMessageWindow(msgwindow)
  }
})
...
MenuHandlers.add(:debug_menu, :item_ball_print_hidden_mode, {
  ...
  "description" => _INTL("Include hidden items in printing?"),
  "effect"      => proc{
