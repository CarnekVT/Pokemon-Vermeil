...
# Layout and position constants (adjust if resolution changes)
ICONS_PER_ROW = 7         # Number of icons per row
ICON_SPACING = 64         # Horizontal and vertical spacing (px) between icons
ICON_LEFT_OFFSET = 28     # X offset from the left edge of the base panel for icons
ICON_TOP_OFFSET = 100     # Y offset from the top edge of the base panel for icons
ARROW_Y_DIVISOR = 16      # Divisor applied to the arrow sprite height for vertical positioning
...
class EncounterList_Scene
  ...
  def pbStartScene
    unless File.file?("Graphics/UI/EncounterUI/"+WINDOWSKIN)
      raise _INTL("You are missing the graphics for this UI. Make sure the image is in Graphics/UI/EncounterUI folder and has the correct name.")
    end
  ...
  def drawPresent
    ...
    enc_array.each do |s|
      species_data = GameData::Species.get(s) # IF NOT SEEN
      ...
      elsif SHOW_SHADOWS_FOR_UNSEEN_POKEMON && !$player.owned?(species_data) # IF NOT CAUGHT
    ...
    loctext += sprintf("<al><c2=7FFF5EF7>Total encounters in area: %s</c2></al>",enc_array.length)
  ...
  def drawAbsent
    ...
    loctext += sprintf("<al><c2=7FFF5EF7>No wild Pokémon in this area</c2></al>")
    loctext += sprintf("<c2=63184210>-----------------------------------------</c2>")
    @sprites["locwindow"].setText(loctext)
  end

  # Method that returns an array of symbolic names for chosen encounter type on current map
  # Currently, ordered by appearance chance and then by national Pokédex number
  def getEncData
    ...
    if encounters
      ...
      # Sort by appearance probability descending
      enc_array.sort_by! { |e| 
