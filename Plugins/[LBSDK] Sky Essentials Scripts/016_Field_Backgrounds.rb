...
class Battle::Scene
  ...
  def pbDeleteField
    if @sprites["terrain_bg"]&.visible
      20.times do
        @sprites["terrain_bg"].opacity -= 8
        sceneWait(2)
      end
    end
    # Clear base sprites if they exist
    if @sprites["base_terrain_0"]
      @sprites["base_terrain_0"].visible = false
      @sprites["base_terrain_0"].dispose
      @sprites.delete("base_terrain_0")
    end
    if @sprites["base_terrain_1"]
      @sprites["base_terrain_1"].visible = false
      @sprites["base_terrain_1"].dispose
      @sprites.delete("base_terrain_1")
    end
    # Clear terrain bg sprite if it exists
  ...
  def pbSetFieldBackground
    # Call the continuous terrain animation system
    if @battle.field.terrain == :None
      pbStopTerrainAnimation if ContinuousTerrainSettings::ENABLED
    else
      # Only start terrain animation if battle is still active
      pbStartTerrainAnimation(@battle.field.terrain) if ContinuousTerrainSettings::ENABLED && !@battleEnd
    end
...
class Battle
  def on_terrain_start
    @scene.pbSetFieldBackground unless @scene.instance_variable_get(:@battleEnd)
  end

  alias pbEOREndTerrain_bg pbEOREndTerrain
  def pbEOREndTerrain
    pbEOREndTerrain_bg
    @scene.pbSetFieldBackground unless @scene.instance_variable_get(:@battleEnd)
  end
