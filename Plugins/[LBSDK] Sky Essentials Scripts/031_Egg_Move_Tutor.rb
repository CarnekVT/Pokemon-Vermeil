# Egg Move Tutor
# by Skyflyer
# For usage instructions, see the example NPC on map 12 (Lugano Condominium).
...
class UI::EggMoveTutorVisuals < UI::MoveReminderVisuals
  ...
  def draw_header
    draw_text(_INTL("Which egg move should I teach?"), HEADER_X, HEADER_Y, theme: :header)
  end
  ...
  def main
    ...
    loop do
      ...
      break if command == :quit && (@mode == :normal ||
               show_confirm_message(_INTL("Stop teaching {1} a move?", @pokemon.name)))
      perform_action(command)
      if @moves.empty?
        show_message(_INTL("{1} has no more egg moves to learn.", @pokemon.name))
        break
      end
      if @disposed
        @result = true
