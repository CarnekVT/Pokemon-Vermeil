class BitmapSprite
  def add_text_theme(theme, base_color, shadow_color)
    @text_themes ||= {}
    @text_themes[theme] = [base_color, shadow_color]
  end

  def draw_themed_text(string, x, y, align = :left, theme = :default, outline = :shadow)
    @text_themes ||= {}
    colors = @text_themes[theme] || [Color.new(64, 64, 64), Color.new(176, 176, 176)]
    base_color, shadow_color = colors
    if outline == :shadow
      # Draw shadow
      self.bitmap.font.color = shadow_color
      case align
      when :left
        self.bitmap.draw_text(x + 1, y + 1, 400, 24, string)
      when :center
        text_width = self.bitmap.text_size(string).width
        self.bitmap.draw_text(x - text_width / 2 + 1, y + 1, text_width, 24, string, 1)
      when :right
        text_width = self.bitmap.text_size(string).width
        self.bitmap.draw_text(x - text_width + 1, y + 1, text_width, 24, string, 2)
      end
    end
    # Draw base text
    ...
    case align
    when :left
      self.bitmap.draw_text(x, y, 400, 24, string)
    when :center
      text_width = self.bitmap.text_size(string).width
      self.bitmap.draw_text(x - text_width / 2, y, text_width, 24, string, 1)
    when :right
      text_width = self.bitmap.text_size(string).width
      self.bitmap.draw_text(x - text_width, y, text_width, 24, string, 2)
    end
  end

  def draw_image(filename, x, y, src_x = 0, src_y = 0, src_width = -1, src_height = -1)
    if filename.is_a?(Bitmap)
      bmp = filename
    elsif filename.is_a?(AnimatedBitmap)
      bmp = filename.bitmap
    else
      bmp = AnimatedBitmap.new(filename).bitmap
    end
    src_rect = Rect.new(src_x, src_y, src_width, src_height)
    src_rect.width = bmp.width - src_x if src_width < 0
    src_rect.height = bmp.height - src_y if src_height < 0
    self.bitmap.blt(x, y, bmp, src_rect)
  end
end
