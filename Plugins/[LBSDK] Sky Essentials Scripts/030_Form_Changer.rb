...
def change_pokemon_form(force_species = nil, message_var = nil, show_messages = true)
  ...
  if $game_variables[1] == -1
    text = _INTL("No? Do you really not want to try a different version of any of your Pokémon?")
    pbMessage(text) if show_messages
    pbSet(message_var, _INTL("No? Do you really not want to try a different version of any of your Pokémon?")) if message_var
    return false 
  end
  ...
  if forms.empty?
    text = _INTL("{1} has no alternative forms available.", pokemon.name)
    pbMessage(text) if show_messages
    pbSet(message_var, _INTL("There are no alternative forms available for {1}.", pokemon.name)) if message_var
    return false if forms.empty?
  end

  if SHOW_SPRITES_IN_FORM_CHANGER
    form_names = forms.map { |form|
      name = !form.form_name || form.form_name.empty? ? _INTL("Normal Form") : form.form_name
      bitmap = GameData::Species.sprite_bitmap(form.species, form.form)
      [name, bitmap]
    }
    form_names << [_INTL("Cancel"), nil]
  else
    form_names = forms.map { |form| 
      !form.form_name || form.form_name.empty? ? _INTL("Normal Form") : form.form_name 
    }
    form_names << _INTL("Cancel")
  end
  
  form_index = pbMessage(_INTL("What form do you want {1} to have?", pokemon.name), form_names, -1)
  if form_index != -1 && form_index < forms.length
    ...
    text = _INTL("Done! I changed {1}'s form to {2}!", pokemon.name, form_name)
