...
#-------------------------------------------------------------------------------
# Exp Share System (from Sky Base) - Complete Implementation
#-------------------------------------------------------------------------------
...
class PokemonGlobalMetadata
  ...
  alias lbdsk_expshare_init initialize unless method_defined?(:lbdsk_expshare_init)
  def initialize
    lbdsk_expshare_init
...
if Settings::USE_NEW_EXP_SHARE
  ...
  class Pokemon
    ...
    def initialize(species, level, player = $player, withMoves = true, recheck_form = true)
      lbdsk_expshare_init(species, level, player, withMoves, recheck_form)
  ...
  class PokemonPartyPanel
    ...
    def initialize(pokemon, index, viewport = nil)
      lbdsk_expshare_init(pokemon, index, viewport)
...
#-------------------------------------------------------------------------------
# JSON helpers (used by Options UI and UI Editor)
#-------------------------------------------------------------------------------
MKXPError = StandardError unless defined?(MKXPError)

# Proporcionar HTTPLite::JSON si no existe
unless defined?(HTTPLite::JSON)
  module HTTPLite
    module JSON
      def self.parse(str)
        if defined?(::JSON)
        ...
        # Fallback: implementación básica de parseo JSON
        parse_json_fallback(str)
      end

      # Implementación básica de parseo JSON para MKXP
      def self.parse_json_fallback(str)
        str = str.strip
        ...
        raise MKXPError, "Unable to parse JSON: #{str[0..50]}"
      end
    end
  end
end
...
unless defined?(JSON)
  module JSON
    ...
    unless respond_to?(:pretty_generate)
...
unless defined?(get_text_colors_for_windowskin)
  def get_text_colors_for_windowskin(windowskin, color, isDarkSkin, no_ctag = true)
    ...
    textcolors = [
      [  0, 112, 248], [120, 184, 232],
      [232,  32,  16], [248, 168, 184],
      [ 96, 176,  72], [174, 208, 144],
      [ 72, 216, 216], [168, 224, 224],
      [208,  56, 184], [232, 160, 224],
      [232, 208,  32], [248, 232, 136],
      [160, 160, 168], [208, 208, 216],
      [240, 240, 248], [120, 120, 128],
      [114, 64, 232], [184, 168, 224],
      [248, 152,  24], [248, 200, 152],
      ...
      [120, 120, 128], [240, 240, 248],
      [138,  90,  43], [216, 184, 154],
      [240,  96, 144], [248, 184, 200],
      [152, 224,  32], [216, 240, 168],
      [ 32,  56, 112], [144, 168, 216],
      [140,  32,  32], [216, 160, 160],
      [ 32, 128, 112], [152, 200, 184],
      [ 88, 168, 240], [184, 216, 248],
      [128, 128,  32], [200, 200, 160],
      [224, 200, 160], [248, 232, 208],
      [184, 144, 224], [224, 208, 248],
      [216, 176,  32], [240, 224, 168],
      [240, 160,  32], [248, 216, 168],
      [248, 160, 144], [248, 216, 200],
      [200,  32,  48], [240, 160, 168],
      [248, 120,  88], [248, 200, 184],
      [ 64,  64, 168], [184, 184, 232],
      [ 96, 120, 160], [192, 208, 232],
      [ 96, 216, 168], [200, 240, 224],
      [ 32,  96,  32], [152, 192, 152],
      [112,  32,  64], [200, 160, 184],
      [168, 144, 120], [224, 208, 200],
      [128, 144, 168], [208, 216, 232]
    ]
...
#-------------------------------------------------------------------------------
# Fix for old Save Files from Sky Base that included DexNav
#-------------------------------------------------------------------------------
class EncounterDexNavLiteData; end
