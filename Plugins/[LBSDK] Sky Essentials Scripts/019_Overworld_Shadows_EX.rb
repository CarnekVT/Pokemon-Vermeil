...
module OWShadowSettings
  ...
  SHADOWLESS_EVENT_NAME     = [
    ...
    "CutTree", "ArbolCorte", "HeadbuttTree", "ArbolGolpeCabeza", "BerryPlant", "Planta Bayas", ".shadowless", ".noshadow", "Entrada Mazmorra Bosque", "Entrada Cueva", "Relic Stone",
    "Escalera", "Puerta","Windmill"
  ]
  ...
  # Disable the automatic (pixel-analysis) generation of shadow sizes.
  # If true, the plugin will use a vanilla default size and allow per-species
  # scaling/overrides via the hashes below.
  DISABLE_AUTO_SIZE = true

  # Default logical width used when auto sizing is disabled.
  DEFAULT_SHADOW_WIDTH = 16

  # Per-species scale multiplier to increase/decrease the default shadow size.
  # Key: species symbol (e.g., :STEELIX). Value: multiplier (float).
  SPECIES_SHADOW_SCALE = {
    STEELIX: 1.8
    # Add more species here, e.g. :LUGIA => 1.6
  }

  # Per-species absolute width override (takes precedence over scale if present).
  # Key: species symbol, Value: integer logical width.
  SPECIES_SHADOW_WIDTH = {
    # Example: :STEELIX => 28
  }
end
...
class Sprite_OWShadow
  ...
  def generate_shadow_data(row_direction)
    ...
    # If automatic sizing is disabled, use default width and allow per-species
    # overrides/scales. Otherwise, analyze the sprite to determine footprint.
    if defined?(OWShadowSettings::DISABLE_AUTO_SIZE) && OWShadowSettings::DISABLE_AUTO_SIZE
      real_width = nil
      offset_x = 0
      logical_width = OWShadowSettings::DEFAULT_SHADOW_WIDTH

      # Try to detect species for per-species overrides (following Pokémon or
      # events that expose `species` or `pokemon`).
      species_sym = nil
      if defined?(Game_FollowingPkmn) && @event.respond_to?(:pokemon) && @event.pokemon
        begin; species_sym = @event.pokemon.species; rescue; species_sym = nil; end
      elsif @event.respond_to?(:species)
        begin; species_sym = @event.species; rescue; species_sym = nil; end
      end

      if species_sym
        species_sym = species_sym.to_sym rescue species_sym
        # Absolute width override takes precedence
        if OWShadowSettings::SPECIES_SHADOW_WIDTH[species_sym]
          logical_width = OWShadowSettings::SPECIES_SHADOW_WIDTH[species_sym]
        elsif OWShadowSettings::SPECIES_SHADOW_SCALE[species_sym]
          logical_width = (logical_width * OWShadowSettings::SPECIES_SHADOW_SCALE[species_sym]).ceil
        end
      end
    else
      # Analyze sprite footprint for smart sizing
      real_width, offset_x = analyze_footprint(bitmap, sx, sy, cw, ch)
      # --- Draw Bitmap ---
      logical_width = (real_width * 0.9 / 2).ceil + 4
    end
    
    # Apply Character Radius Fix (filename-based) and keep it when auto sizing
    # is disabled as well.
