...
def openPokedexOnPokemon(species, gender, form = 0)
  ...
  # Clamp region to valid town map regions
  if region >= 0
    max_town_region = GameData::TownMap.keys.max || 0
    region = max_town_region if region > max_town_region
  end
  ...
  if dexlist[index][:species] != species
    return pbMessage(_INTL("{1} not found in the Pokédex.", GameData::Species.get(species).name))
  end
