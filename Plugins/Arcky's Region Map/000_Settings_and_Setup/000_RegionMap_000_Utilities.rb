SaveData.register(:arcky_region_map_global) do
  load_in_bootup
  save_value { $ArckyRegionMapGlobal }
  load_value { |value| $ArckyRegionMapGlobal = value }
  new_game_value { ArckyRegionMapGlobalData.new }
  reset_on_new_game
end

class ArckyRegionMapGlobalData
...
# Compatibility shim for older saves that reference ArckyGlobalData.
unless defined?(ArckyGlobalData)
  class ArckyGlobalData < ArckyRegionMapGlobalData
  end
end
...
module ARMUtils
  ...
  def global
    $ArckyRegionMapGlobal ||= ArckyRegionMapGlobalData.new
  end
  ...
  def getSpeciesDexNumber(pokemon, region)
    ...
    if num <= 0
      ...
      dexnum += (speciesData.form.to_f / 10) if speciesData.form > 0
    end
  ...
  def getSpeciesTypes(speciesData)
    ...
    typeNames = type.map { |t| GameData::Type.try_get(t).name }
    typeNames.size > 1 ? typeNames.join("/") : typeNames.first
  end
  ...
  def getDistrictProgress
    return if global.globalCounter.empty?
    array = {}
    global.globalCounter[:districts].each do |district, progress|
  ...
  def getCounters(type, mapID)
    case type
    when SEEN
      return [global.seenSpeciesCount, global.seenSpeciesCountMap,
              global.lastSeenSpeciesForm, global.lastSeenSpeciesFormMap]
    when CAUGHT
      return [global.caughtSpeciesCount, global.caughtSpeciesCountMap,
              global.lastCaughtSpeciesForm]
    when DEFEATED
      return [global.defeatedSpeciesCount, global.defeatedSpeciesCountMap]
    end
  end
  ...
  def countSpeciesForms(counter)
    ...
    array = counter.map { |sar| sar.map { |ssar| ssar.compact } }.flatten(1)
    maxLength = array.map(&:length).max
    sums = Array.new(maxLength, 0)
    array.each { |sarr| sarr.each_with_index { |value, index| sums[index] += value } }
    return sums.length
  end
